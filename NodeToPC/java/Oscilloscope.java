...
public class Oscilloscope implements MessageListener
{
    ...
    if (msg instanceof OscilloscopeMsg) {
        ...
        long time, token = 2823402076l;

				System.out.println(omsg.get_token());
				
        if (omsg.get_token() != token) {
					return;
        }

        nodeID = omsg.get_nodeID();
        seq = omsg.get_seq();
        temp = omsg.get_temp();
        humid = omsg.get_humid();
        light = omsg.get_light();
        time = omsg.get_time();
        version = omsg.get_version();
        interval = omsg.get_interval();

        try {
					BufferedWriter bw = new BufferedWriter(new FileWriter(new File("../result.txt"), true));
					bw.write(nodeID + " " + seq + " " + temp + " " + humid + " " + light + " " + time + "\n");
					//bw.write(nodeID + " " + seq + " " + temp + " " + humid + " " + light + " " + time + " " + version + " " + interval + " " + omsg.get_token() + "\n");
					bw.flush();
					bw.close();
				} catch (IOException e) {
						/* Skip Field */
        ...
        data.update(omsg.get_nodeID(), omsg.get_seq(), omsg.get_temp(), omsg.get_humid(), omsg.get_light());
        //System.out.println("seq: " + omsg.get_seq() + " temp: " + omsg.get_temp());
    ...
    if (moteVersion > version) {
        ...
        //System.out.println("moteVersion: " + moteVersion + " version: " + version);
        //version = moteVersion;
    ...
    void sendInterval() {
    System.out.println("sendInterval " + " version: " + version + " interval: " + interval);
