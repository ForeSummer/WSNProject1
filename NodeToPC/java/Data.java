...
class Data {
    ...
    void update(int nodeId, int messageId, int temp, int humid, int light) {
    ...
    node.update(messageId, temp, humid, light);
    }

    /* Return value of sample x for mote nodeId, or -1 for missing data */
    int getTempData(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null) {
    		//System.out.println("Data   missing data for nodeId: " + nodeId);
        return -1;
    }
    return nodes[nodeId].getTempData(x);
    }

    int getHumidData(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null)
        return -1;
    return nodes[nodeId].getHumidData(x);
    }

    int getLightData(int nodeId, int x) {
    if (nodeId >= nodes.length || nodes[nodeId] == null)
        return -1;
    return nodes[nodeId].getLightData(x);
    }
